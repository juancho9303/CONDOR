...
def density_profile_linear(x, y, s_d, r_d_min, x0, y0, inc_phot, pa_phot):
...
def density_profile_convolved(x, y, s_d, r_d_min, x0, y0, inc_phot, pa_phot, conv_kernel):
...
#-----------------------------------------------------------------------------------------------------------------------
#-------------------------------------------------- MCMC individual case -----------------------------------------------
#-----------------------------------------------------------------------------------------------------------------------

def lnprior_one_gal(param, x0_guess, y0_guess):
    ...
    if (0 < pa < 360
            ...
            and 0.1 < rflat < 5
            and 50 < vflat < 380
            and x0_guess - 0.5 < x0 < x0_guess + 0.5
            and y0_guess - 0.5 < y0 < y0_guess + 0.5):
        return 0.0
...
def Lg_one_gal(param, vel, x, y, NS_kernel, AO_kernel, i, var):
    pa, inc, rflat, vflat, x0, y0 = param

    prob = np.nansum(- (vel - my_vel_model(x, y, pa, inc, rflat, vflat, x0, y0, NS_kernel, AO_kernel, i,
                                                 vel)[7]) ** 2 / (2 * var))
...
def lnprob_one_gal(param, vel, x, y, NS_kernel, AO_kernel, i, var, x0_guess, y0_guess):
    lp = lnprior_one_gal(param, x0_guess, y0_guess)

    if not np.isfinite(lp):
        return -np.inf
    return lp + Lg_one_gal(param, vel, x, y, NS_kernel, AO_kernel, i, var)
...
def lnprior_combine(param, x0_guess_ns, y0_guess_ns, x0_guess_ao, y0_guess_ao):

    pa, inc, rflat, vflat, x0_ns, y0_ns, x0_ao, y0_ao = param

    if (0 < pa < 360
            and 20 < inc < 90
            # and -2.79   < inc   < -0.35           # from -160 to -20
            and 0.1 < rflat < 5.5
            and 50 < vflat < 450
            and x0_guess_ns - 1.5 < x0_ns < x0_guess_ns + 1.5
            and y0_guess_ns - 1.5 < y0_ns < y0_guess_ns + 1.5
            and x0_guess_ao - 1.5 < x0_ao < x0_guess_ao + 1.5
            and y0_guess_ao - 1.5 < y0_ao < y0_guess_ao + 1.5):
        return 0.0
...
def Lg_combine(param, vel_ns, x_ns, y_ns, x_ao, y_ao, var_ns, vel_ao, NS_kernel, AO_kernel, i, var_ao, resolution_ratio):
    pa, inc, rflat, vflat, x0_ns, y0_ns, x0_ao, y0_ao = param


    prob = np.nansum( - (vel_ns - my_vel_model(x_ns, y_ns, pa, inc, (rflat/resolution_ratio), vflat, x0_ns, y0_ns,
                                               NS_kernel, 0, i, vel_ns)[7]) ** 2
                        / (2 * var_ns)) \
            + np.nansum( - (vel_ao - my_vel_model(x_ao, y_ao, pa, inc, rflat, vflat, x0_ao, y0_ao, 0, AO_kernel, i,
                                                  vel_ao)[7]) ** 2
                        / (2 * var_ao))
...
def lnprob_combine(param, vel_ns, x_ns, y_ns, x_ao, y_ao, var_ns, vel_ao, NS_kernel, AO_kernel, i, var_ao,
                   x0_guess_ns, y0_guess_ns, x0_guess_ao, y0_guess_ao, resolution_ratio):

    lp = lnprior_combine(param, x0_guess_ns, y0_guess_ns, x0_guess_ao, y0_guess_ao)

    if not np.isfinite(lp):
        return -np.inf
    return lp + Lg_combine(param, vel_ns, x_ns, y_ns, x_ao, y_ao, var_ns, vel_ao, NS_kernel, AO_kernel, i, var_ao,
                           resolution_ratio)
...
def ns_maps(i, inputs, constant_inputs):
    ...
    vel_data, pixscale, Da, r_d_guess, l0, wav, NS_kernel, AO_kernel, z, x0_fixed, y0_fixed, inc_fixed, rflat_lim, vflat_max = constant_inputs
    y, x = np.indices([vel_data.shape[0], vel_data.shape[1]])
    kpc_per_pix = pixscale[0] / 3600 / 180 * np.pi * Da * 1000
    #rflat = rflat * (pixscale[1] / pixscale[0])
...
def ao_maps(i, inputs, constant_inputs):
    ...
    vel_data, pixscale, Da, r_d_guess, l0, wav, NS_kernel, AO_kernel, z, x0_fixed, y0_fixed, inc_fixed, rflat_lim, vflat_max = constant_inputs
    y, x = np.indices([vel_data.shape[0], vel_data.shape[1]])
    kpc_per_pix = pixscale[1] / 3600 / 180 * np.pi * Da * 1000  # Calculate kpc per pixel
    #rflat = rflat / kpc_per_pix
    r_d_min = r_d_guess
...
# Log prior
def lnprior_cube_unconstrained(param, x0_fixed, y0_fixed, inc_fixed, rflat_lim, vflat_max):
    pa, inc, rflat, vflat, x0, y0 = param
    ...
    if (0 < pa < 360
            ...
            and 0.1 < rflat < rflat_lim
            and 50 < vflat < 1200
            and (x0_fixed - 2.5) < x0 < (x0_fixed + 2.5)
            and (y0_fixed - 2.5) < y0 < (y0_fixed + 2.5)):
        return np.log(np.sin(np.radians(inc))) - np.log(vflat) - np.log(rflat)
...
# The natural logarithm of the joint likelihood
def Lg_cube_unconstrained(param, vel, x, y, model, constant_inputs, i, var):
    pa, inc, rflat, vflat, x0, y0 = param
    input_params = [pa, inc, rflat, vflat, x0, y0, x0, y0]
...
# log-probability function (log of the joint posterior)
def lnprob_cube_unconstrained(param, vel, x, y, model, constant_inputs, i, var):
    x0_fixed, y0_fixed, inc_fixed = constant_inputs[-5], constant_inputs[-4], constant_inputs[-3]
    rflat_lim = constant_inputs[-2]
    vflat_max = constant_inputs[-1]

    lp = lnprior_cube_unconstrained(param, x0_fixed, y0_fixed, inc_fixed, rflat_lim, vflat_max)
    if not np.isfinite(lp):
        return -np.inf
    # print(lp, Lg_cube(param, vel, x, y, model, constant_inputs, i, var), lp + Lg_cube(param, vel, x, y, model, constant_inputs, i, var))
    return lp + Lg_cube_unconstrained(param, vel, x, y, model, constant_inputs, i, var)

#------------------------------------------#### CUBE combine unconstrained ####-------------------------------------------------------

def lnprior_combine_cube_unconstrained(param, x0_ns_fixed, y0_ns_fixed, x0_ao_fixed, y0_ao_fixed, inc_fixed, rflat_lim, vflat_max):
    ...
    if (0 < pa < 360
        ...
        and 0.1 < rflat < rflat_lim
        and 50 < vflat < 1200
        and (x0_ns_fixed - 2.5) < x0_ns < (x0_ns_fixed + 2.5)
        and (y0_ns_fixed - 2.5) < y0_ns < (y0_ns_fixed + 2.5)
        and (x0_ao_fixed - 2.5) < x0_ao < (x0_ao_fixed + 2.5)
        and (y0_ao_fixed - 2.5) < y0_ao < (y0_ao_fixed + 2.5)):
...
def Lg_combine_cube_unconstrained(param, vel_ns, x_ns, y_ns, var_ns, model_ns, constant_inputs_ns, vel_ao, x_ao, y_ao, var_ao,
                    model_ao, constant_inputs_ao, i, resolution_ratio):
...
def lnprob_combine_cube_unconstrained(param, vel_ns, x_ns, y_ns, var_ns, model_ns, constant_inputs_ns, vel_ao, x_ao, y_ao, var_ao,
                        model_ao, constant_inputs_ao, i, resolution_ratio):
    x0_ns_fixed, y0_ns_fixed = constant_inputs_ns[-5], constant_inputs_ns[-4]
    inc_fixed = constant_inputs_ns[-3]
    x0_ao_fixed, y0_ao_fixed = constant_inputs_ao[-5], constant_inputs_ao[-4]
    ...
    lp = lnprior_combine_cube_unconstrained(param, x0_ns_fixed, y0_ns_fixed, x0_ao_fixed, y0_ao_fixed, inc_fixed, rflat_lim,vflat_max)

    if not np.isfinite(lp):
        return -np.inf

    return lp + Lg_combine_cube_unconstrained(param, vel_ns, x_ns, y_ns, var_ns, model_ns, constant_inputs_ns, vel_ao, x_ao, y_ao,
                                var_ao, model_ao, constant_inputs_ao, i, resolution_ratio)
